...
class account_invoice(osv.osv):
    ...
    @api.onchange('entrega_factura_fisica')
    def check_entrega_factura_fisica_change(self):
        print(self._context)
        if 'id' in self._context.get('params'):
            obj_val_p = self.env['modulo_valorizaciones.proveedor'].search([['n_factura', '=', self._context.get('params')['id']]], limit=1)
            print (obj_val_p)
            if self.entrega_factura_fisica:
                obj_val_p.write({'estado': 'Facturado'})
            else:
                obj_val_p.write({'estado': 'No Emitido'})
    ...
    def action_enviar_sunat(self, context=None):
        ...
        for invoice in self.env['account.invoice'].browse(self.id):
            ...
            buscartareo = self.env['modulo_valorizaciones.tareo_mensual'].search([['n_factura', '=', invoice.id]])
            buscarnombre = self.env['modulo.valorizaciones.pdf.clientes'].search([['cliente', '=', buscartareo.cliente_id.id], ['fecha_inicio', '=', buscartareo.fechaInicio], ['fecha_fin', '=', buscartareo.fechaFin], ['fecha_facturacion', '=', buscartareo.fechaFacturacion]], limit=1)

            filename_valorizacion = str(buscarnombre.url_pdf) # url del pdf de valorizacion
