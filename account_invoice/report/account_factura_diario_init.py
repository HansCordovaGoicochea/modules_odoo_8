...
class general_factura_diario(report_sxw.rml_parse):
    ...
    def get_data_fac(self, id_wizard, fi, ff, filter2):
        print ('------------')
        print (id_wizard.id, fi, ff, filter2)
        print ('------------')

        if filter2 == 'filter_period':
            sql = """
                               SELECT ai.*, rp.name as nombre_proveedor, rp.display_name as nombre_proveedor2, 
                               rp.doc_type, rp.doc_number, tc_venta, tc_compra, rc.name as codecambio, rc.id as idcurrency, ai.comment, cast('' as text) as r_fecha, cast(ec.code as text) as r_doc, '' as r_num, '' as descripcion FROM account_invoice ai 
                               LEFT JOIN res_partner rp ON  ai.partner_id = rp.id 
                               LEFT JOIN res_currency rc ON  ai.currency_id = rc.id 
                               LEFT JOIN einvoice_catalog_01 ec ON  ec.id = ai.tipo_documento_ref
                               where tipo_factura = %s and period_id = %s and ai.type = 'in_invoice' and ai.state in ('open','paid')
                               ORDER by ai.id
                           """
            self.cr.execute(sql, (id_wizard.id, fi))
        else:
            sql = """
                                           SELECT ai.*, rp.name as nombre_proveedor, rp.display_name as nombre_proveedor2, 
                                           rp.doc_type, rp.doc_number, tc_venta, tc_compra, rc.name as codecambio, rc.id as idcurrency, ai.comment, cast('' as text) as r_fecha, cast(ec.code as text) as r_doc, '' as r_num, '' as descripcion FROM account_invoice ai 
...
class general_factura_diario_compras(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        super(general_factura_diario_compras, self).__init__(cr, uid, name, context)
    ...
    def get_data_fac(self, id_wizard, fi, ff, filter2):
        ...
        if filter2 == 'filter_period':
            sql = """
                               ...
                               LEFT JOIN res_currency rc ON  ai.currency_id = rc.id 
                               LEFT JOIN einvoice_catalog_01 ec ON  ec.id = ai.tipo_documento_ref
                               where tipo_factura = %s and period_id >= %s and period_id <= %s and ai.type = 'in_invoice' and ai.state in ('open','paid')
                               ORDER by ai.id
                           """
        else:
            sql = """
...
class report_factura_diario_compras(osv.AbstractModel):
    _name = 'report.account_invoice.report_facturas_diario_pdf'
    _inherit = 'report.abstract_report'
    _template = 'account_invoice.report_facturas_diario_pdf'
    _wrapped_report_class = general_factura_diario_compras
# Fin reportecompras 2
...
class general_factura_diario_ventas(report_sxw.rml_parse):
    ...
    def get_data_fac(self, id_wizard, fi, ff):
        ...
        sql = """
                   ...
                   LEFT JOIN res_partner rp ON  ai.partner_id = rp.id 
                   LEFT JOIN res_currency rc ON  ai.currency_id = rc.id 
                   where CAST(coalesce(aj.code, '0') AS integer) = %s and  aj.type = 'sale' and  ai.type = 'out_invoice' and ai.state in ('open','paid','anulado','cancel','error_rechazo') and date_invoice >= %s and date_invoice <= %s
                   UNION
                   ...
                   LEFT JOIN res_partner rp ON  ai.partner_id = rp.id 
                   LEFT JOIN res_currency rc ON  ai.currency_id = rc.id 
                   where CAST(coalesce('7', '0') AS integer) = %s and  aj.type = 'sale' and  ai.type = 'out_invoice' and ai.state in ('open','paid','anulado','cancel','error_rechazo') and fecha_emision >= %s and fecha_emision <= %s and einc.state != 'error' ORDER BY internal_number
               """
        # print(sql)
