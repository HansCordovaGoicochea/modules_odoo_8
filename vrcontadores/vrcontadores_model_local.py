    ...
    if monto_pagar and tipo_pago:
        if (tipo_pago == 'trabajo'):
            ...
            if fecha_incio:
                ...
                delete = "DELETE FROM res_partner_pagos_cliente WHERE cliente=" + str(cliente) + "AND estado = 'deudor' AND fecha > '"+fecha_incio+"'"
                self._cr.execute(delete)
                # if est == 'pago':
                #
                # elif est == 'act':
                ...
                while int(fecha[0]) <= int(dateMonthEnd[0:4]):
                    ...
                    for i in range(0, int(13-int(fecha_incio[5:7]))):
                        ...
                        # print ("Nueva Fecha = " , str(nueva_fecha))
...
class res_partner(models.Model):
    ...
    tipo_pago = fields.Selection(
            [('quincenal', 'Quincenal'), ('mensual', 'Mensual'), ('semanal', 'Semanal'), ('diario', 'Diario'), ('trabajo', 'Por Trabajo'),
    ...
    estado_contribuyente = fields.Selection([('ACTIVO', 'ACTIVO'), ('BAJA TEMPORAL', 'BAJA TEMPORAL'), ('BAJA DEFINITIVA', 'BAJA DEFINITIVA'), ('BAJA DE OFICIO', 'BAJA DE OFICIO'), ('SUSPENSION TEMPORAL', 'SUSPENSION TEMPORAL')],
        'Estado del contribuyente')
    condicion_contribuyente = fields.Selection([('HABIDO', 'HABIDO'), ('NO HABIDO', 'NO HABIDO'), ('NO HALLADO', 'NO HALLADO')],
    ...
    def sunat_2(self):
        from sunat import sunatSelenium
        url = sunatSelenium()
        url.sunatDos(str(self.doc_number) if self.doc_number else '', str(self.usuario) if self.usuario else '',str(self.clave) if self.clave else '')
    ...
    def estado_cliente(self, estado, condicion):
        today = datetime.datetime.now()
        id_estado = self.pool.get('res.partner.reporte_estado').search(self.env.cr, self.env.uid, [('parent_id', '=', self.id)])
    ...
    def load_pantalla_impuestos_pendientes(self):
        ...
        if existeReportePagosAnio:
            parent = self.pool.get('res.partner.reporte_pagos').browse(self.env.cr, self.env.uid, existeReportePagosAnio,
                                                        context=None).id
        else:
            vals = \
                {'name': today.year
        ...
        if existe:
            ...
            res_id = self.pool.get('res.partner.impuestos_pendientes').create(self.env.cr, self.env.uid, vals, context=ctx)
            self.createDetalleImpuestos()
            return {
                'name': 'Impuestos Pendientes',
                'type': 'ir.actions.act_window',
    ...
    def createDetalleImpuestos(self):
        existe = self.pool.get('res.partner.impuestos_pendientes').search(self.env.cr, self.env.uid,
                                                                          [('parent_id', '=',
                                                                            self.id)])
        impuesto = self.pool.get('res.partner.impuestos_pendientes').browse(self.env.cr, self.env.uid, existe, context=None)
        ...
        detalle_impuesto = self.pool.get('res.partner.impuestos_pendientes_detalle').browse(self.env.cr, self.env.uid, existeDetalle, context=None)
    ...
    def write_fechas(self, vals, context=None):
        ...
        if fecha_incio:
            self._cr.execute("""SELECT COUNT(id) FROM res_partner_pagos_cliente WHERE estado = %s and fecha >= %s and cliente = %s""", ('pagado', fecha_incio, self.id))
    ...
    def load_pantalla_honorarios(self):
        ...
        existeReporteHonorario = self.pool.get('res.partner.reporte_honorarios').search(self.env.cr, self.env.uid,[('parent_id', '=',
self.id)])
        if existeReporteHonorario:
    ...
    def load_pantalla_documentos(self):
        ...
        if existeReporteHonorarioAnio:
            parent = self.pool.get('ir.ui.view').browse(self.env.cr, self.env.uid, existeReporteHonorarioAnio, context=None).id
        else:
            vals = \
                {'name': today.year
                 }
            parent = self.pool.get('res.partner.reporte_documentos').create(self.env.cr, self.env.uid, vals, context=None)
...
class res_partner_historia_clinica(models.Model):
    ...
    parent_catalogo_ids = fields.One2many(comodel_name="res.partner.historia_clinica_catalogo_meses", inverse_name="parent_catalogo_id", string="Historia por meses", compute='dep_mes_cata', store=True, readonly=False)
    ...
    def mostrarImpuestos(self):
        ...
        idImp = self.pool.get('res.partner.historia_clinica').search(self.env.cr, self.env.uid, [('parent_id', '=', self.parent_id.id)])
        imp = self.pool.get('res.partner.historia_clinica').browse(self.env.cr, self.env.uid, idImp, context=None)
        id = self.pool.get('res.partner.historia_clinica_catalogo_meses').search(self.env.cr, self.env.uid, [('mes', '=', self.mes), ('anio', '=', self.anio), ('cliente', '=', self.parent_id.id)])
        impuestos = self.pool.get('res.partner.historia_clinica_catalogo_meses').browse(self.env.cr, self.env.uid, id,context=None)
...
class res_partner_historia_clinica_detalle(models.Model):
    ...
    def create(self, vals, context=None):
        ...
        print (self._context)
        if vals.has_key('anio') and vals.has_key('fecha') and vals.has_key('mes') and vals.has_key('historico') and vals.has_key('estado'):
            valores = {
                    'anio': vals['anio'],
                    'fecha': vals['fecha'],
                    'mes': vals['mes'],
    ...
    historia_clinica_detalle_ids = fields.One2many('res.partner.historia_clinica_detalle', 'parent_detalle_id', 'Included Services')
...
class res_partner_impuestos_pendientes(models.Model):
    ...
    def mostrarImpuestos(self):
        ...
        impuestos = self.pool.get('res.partner.impuestos_pendientes_detalle').browse(self.env.cr, self.env.uid, id, context=None)
...
class res_partner_impuestos_pendientes_detalle(models.Model):
    ...
    impuesto_pendiente_detalle_ids = fields.One2many('res.partner.impuestos_pendientes_detalle', 'parent_detalle_id', 'Included Services')
    mes = fields.Selection(
        [('1', 'Enero'), ('2', 'Febrero'), ('3', 'Marzo'), ('4', 'Abril'), ('5', 'Mayo'), ('6', 'Junio'),
         ('7', 'Julio'), ('8', 'Agosto'), ('9', 'Setiembre'), ('10', 'Octubre'), ('11', 'Noviembre'),
         ('12', 'Diciembre')], 'Mes', default=_mes_get_fnc)
        ...
        for val in vals['impuesto_pendiente_detalle_ids']:
            if val[2]:
                ...
                for item in valores.keys():
                    if item == 'fecha':
                        upd = "UPDATE res_partner_impuestos_pendientes_detalle SET fecha='" + str(valores['fecha']) + "' WHERE id = " + str(val[1])
                        self._cr.execute(upd)
                    elif item == 'deuda':
                        upd = "UPDATE res_partner_impuestos_pendientes_detalle SET deuda=" + str(valores['deuda']) + " WHERE id =" + str(val[1])
                        self._cr.execute(upd)
...
def get_fecha_limite(self, mes, anio, cliente):
    ...
    id = self.pool.get('res.partner.cronograma_obligaciones').search(self.env.cr, self.env.uid,
                                            [('mes', '=', mes), ('anio', '=', anio)])
    for i in id:
        cronograma = self.pool.get('res.partner.cronograma_obligaciones').browse(self.env.cr, self.env.uid, i, context=None)
...
class res_partner_declaraciones_detalle(models.Model):
    ...
    mes = fields.Selection([('1', 'Enero'), ('2', 'Febrero'), ('3', 'Marzo'), ('4', 'Abril'), ('5', 'Mayo'), ('6', 'Junio'), ('7', 'Julio'), ('8', 'Agosto'), ('9', 'Setiembre'), ('10', 'Octubre'), ('11', 'Noviembre'), ('12', 'Diciembre')], 'Mes', default=_mes_get_fnc)
    anio = fields.Selection(get_rango_anios, 'Año',  default=_anio_get_fnc)
    ...
    def _compute_tot_pagado(self):
        ...
        id = self.pool.get('res.partner.pagos_cliente').search(self.env.cr, self.env.uid,
                                                               [('fecha', '>=', str(nueva_inicio)),
                                                                ('fecha', '<=', str(nueva_fin)),
                                                                ('cliente', '=',self._context.get('default_parent_id'))])
    ...
    def mostrarCronograma(self):
        ...
        id = self.pool.get('res.partner.honorarios_detalle').search(self.env.cr, self.env.uid,
                                                                    [('mes', '=', self.mes),
                                                                     ('anio', '=', self.anio),
                                                                     ('parent_detalle_id', '=',self.id)])
        pagos_honorarios = self.pool.get('res.partner.honorarios_detalle').browse(self.env.cr, self.env.uid, id, context=None)
        for h in pagos_honorarios:
            monto = monto + h.importe_pagado
...
class res_partner_honorarios_detalle(models.Model):
    ...
    #parent_id = fields.Many2one('res.partner', readonly=True)
    fecha_pago = fields.Date('Fecha de Pago')
    mes = fields.Selection([('1', 'Enero'), ('2', 'Febrero'), ('3', 'Marzo'), ('4', 'Abril'), ('5', 'Mayo'), ('6', 'Junio'), ('7', 'Julio'), ('8', 'Agosto'), ('9', 'Setiembre'), ('10', 'Octubre'), ('11', 'Noviembre'), ('12', 'Diciembre')], 'Mes Honorarios', default=_mes_get_fnc)
...
class res_partner_pagos_cliente(models.Model):
    ...
    cliente = fields.Many2one('res.partner')
    ruc = fields.Char('RUC')
    estado = fields.Selection([('deudor', 'Deudor'), ('pagado', 'Pagado')], 'Estado', default='deudor')
...
class res_partner_documentos(models.Model):
    ...
    _inherits = {'res.partner.documentos_quincena': 'documento_quincena_id', 'res.partner.documentos_fin_mes': 'documento_fin_mes_id'}
    _rec_name = 'parent_id'
    parent_id = fields.Many2one('res.partner', readonly=True)
    anio = fields.Selection(get_rango_anios, 'Año de Recepción',  default=_anio_get_fnc)
    mes = fields.Selection([('1', 'Enero'), ('2', 'Febrero'), ('3', 'Marzo'), ('4', 'Abril'), ('5', 'Mayo'), ('6', 'Junio'), ('7', 'Julio'), ('8', 'Agosto'), ('9', 'Setiembre'), ('10', 'Octubre'), ('11', 'Noviembre'), ('12', 'Diciembre')], 'Mes de Recepción', default=_mes_get_fnc)
...
class res_partner_documentos_quincena(models.Model):
    ...
    estado = fields.Selection([('Pendiente', 'Pendiente'), ('Recibido', 'Recibido'), ('No tiene documentos', 'No tiene documentos')], 'Estado', default='Pendiente')
    observaciones = fields.Text('Observaciones')
    parent_detalle_id = fields.Many2one('res.partner.documentos_quincena', 'Parent')
    documento_quincena_ids = fields.One2many('res.partner.documentos_quincena', 'parent_detalle_id', 'Included Services')
...
class res_partner_reporte_honorarios(models.Model):
    ...
    enero = fields.Selection([('Pronto a vencerse', 'Pronto a vencerse'), ('Pagó', 'Pagó'), ('Deudor', 'Deudor'), ('No aplica', 'No aplica')], 'Enero', compute='_compute_enero')
    febrero = fields.Selection([('Pronto a vencerse', 'Pronto a vencerse'), ('Pagó', 'Pagó'), ('Deudor', 'Deudor'), ('No aplica', 'No aplica')], 'Febrero', compute='_compute_febrero')
    marzo = fields.Selection([('Pronto a vencerse', 'Pronto a vencerse'), ('Pagó', 'Pagó'), ('Deudor', 'Deudor'), ('No aplica', 'No aplica')], 'Marzo', compute='_compute_marzo')
    abril = fields.Selection([('Pronto a vencerse', 'Pronto a vencerse'), ('Pagó', 'Pagó'), ('Deudor', 'Deudor'), ('No aplica', 'No aplica')], 'Abril', compute='_compute_abril')
    mayo = fields.Selection([('Pronto a vencerse', 'Pronto a vencerse'), ('Pagó', 'Pagó'), ('Deudor', 'Deudor'), ('No aplica', 'No aplica')], 'Mayo', compute='_compute_mayo')
    junio = fields.Selection([('Pronto a vencerse', 'Pronto a vencerse'), ('Pagó', 'Pagó'), ('Deudor', 'Deudor'), ('No aplica', 'No aplica')], 'Junio', compute='_compute_junio')
    julio = fields.Selection([('Pronto a vencerse', 'Pronto a vencerse'), ('Pagó', 'Pagó'), ('Deudor', 'Deudor'), ('No aplica', 'No aplica')], 'Julio', compute='_compute_julio')
    agosto = fields.Selection([('Pronto a vencerse', 'Pronto a vencerse'), ('Pagó', 'Pagó'), ('Deudor', 'Deudor'), ('No aplica', 'No aplica')], 'Agosto', compute='_compute_agosto')
    setiembre = fields.Selection([('Pronto a vencerse', 'Pronto a vencerse'), ('Pagó', 'Pagó'), ('Deudor', 'Deudor'), ('No aplica', 'No aplica')], 'Setiembre', compute='_compute_setiembre')
    octubre = fields.Selection([('Pronto a vencerse', 'Pronto a vencerse'), ('Pagó', 'Pagó'), ('Deudor', 'Deudor'), ('No aplica', 'No aplica')], 'Octubre', compute='_compute_octubre')
    noviembre = fields.Selection([('Pronto a vencerse', 'Pronto a vencerse'), ('Pagó', 'Pagó'), ('Deudor', 'Deudor'), ('No aplica', 'No aplica')], 'Noviembre', compute='_compute_noviembre')
    diciembre = fields.Selection([('Pronto a vencerse', 'Pronto a vencerse'), ('Pagó', 'Pagó'), ('Deudor', 'Deudor'), ('No aplica', 'No aplica')], 'Diciembre', compute='_compute_diciembre')
    _rec_name = 'parent_id'
    ...
    def getEstado(self, mes):
        ...
        nueva_inicio = date(int('2017'), int(mes), 1)
        nueva_fin = date(int('2017'), int(mes), meses[int(mes) - 1])
        id = self.pool.get('res.partner.pagos_cliente').search(self.env.cr, self.env.uid,
                                                               [('fecha', '>=', str(nueva_inicio)),
                                                                ('fecha', '<=', str(nueva_fin)),
                                                                ('estado', '=', 'deudor'),
        ...
        cliente = self.pool.get('res.cliente.contrato').search(self.env.cr, self.env.uid, [('partner_id', '=', self.parent_id.id)],limit=1,order= "id")
        print('ddddd')
        print(cliente)
        print('ddddd')
        ...
        if contrato:
            slist = contrato.fecha_inicio.split("-")
        else:
            slist = [0,0]
...
class res_partner_reporte_declaracion(models.Model):
    ...
    enero_pdt = fields.Selection([('Declarado a tiempo', 'Declarado a tiempo'), ('Declarado desfasado', 'Declarado desfasado'), ('Pendiente declaración', 'Pendiente declaración')], 'Enero PDT', compute='_compute_enero_pdt')
    febrero_pdt = fields.Selection([('Declarado a tiempo', 'Declarado a tiempo'), ('Declarado desfasado', 'Declarado desfasado'), ('Pendiente declaración', 'Pendiente declaración')], 'Febrero PDT', compute='_compute_febrero_pdt')
    marzo_pdt = fields.Selection([('Declarado a tiempo', 'Declarado a tiempo'), ('Declarado desfasado', 'Declarado desfasado'), ('Pendiente declaración', 'Pendiente declaración')], 'Marzo PDT', compute='_compute_marzo_pdt')
    abril_pdt = fields.Selection([('Declarado a tiempo', 'Declarado a tiempo'), ('Declarado desfasado', 'Declarado desfasado'), ('Pendiente declaración', 'Pendiente declaración')], 'Abril PDT', compute='_compute_abril_pdt')
    mayo_pdt = fields.Selection([('Declarado a tiempo', 'Declarado a tiempo'), ('Declarado desfasado', 'Declarado desfasado'), ('Pendiente declaración', 'Pendiente declaración')], 'Mayo PDT', compute='_compute_mayo_pdt')
    junio_pdt = fields.Selection([('Declarado a tiempo', 'Declarado a tiempo'), ('Declarado desfasado', 'Declarado desfasado'), ('Pendiente declaración', 'Pendiente declaración')], 'Junio PDT', compute='_compute_junio_pdt')
    julio_pdt = fields.Selection([('Declarado a tiempo', 'Declarado a tiempo'), ('Declarado desfasado', 'Declarado desfasado'), ('Pendiente declaración', 'Pendiente declaración')], 'Julio PDT', compute='_compute_julio_pdt')
    agosto_pdt = fields.Selection([('Declarado a tiempo', 'Declarado a tiempo'), ('Declarado desfasado', 'Declarado desfasado'), ('Pendiente declaración', 'Pendiente declaración')], 'Agosto PDT', compute='_compute_agosto_pdt')
    setiembre_pdt = fields.Selection([('Declarado a tiempo', 'Declarado a tiempo'), ('Declarado desfasado', 'Declarado desfasado'), ('Pendiente declaración', 'Pendiente declaración')], 'Setiembre PDT', compute='_compute_setiembre_pdt')
    octubre_pdt = fields.Selection([('Declarado a tiempo', 'Declarado a tiempo'), ('Declarado desfasado', 'Declarado desfasado'), ('Pendiente declaración', 'Pendiente declaración')], 'Octubre PDT', compute='_compute_octubre_pdt')
    noviembre_pdt = fields.Selection([('Declarado a tiempo', 'Declarado a tiempo'), ('Declarado desfasado', 'Declarado desfasado'), ('Pendiente declaración', 'Pendiente declaración')], 'Noviembre PDT', compute='_compute_noviembre_pdt')
    diciembre_pdt = fields.Selection([('Declarado a tiempo', 'Declarado a tiempo'), ('Declarado desfasado', 'Declarado desfasado'), ('Pendiente declaración', 'Pendiente declaración')], 'Diciembre PDT', compute='_compute_diciembre_pdt')
    enero_plame = fields.Selection([('Declarado a tiempo', 'Declarado a tiempo'), ('Declarado desfasado', 'Declarado desfasado'), ('Pendiente declaración', 'Pendiente declaración')], 'Enero PLAME', compute='_compute_enero_plame')
    febrero_plame = fields.Selection([('Declarado a tiempo', 'Declarado a tiempo'), ('Declarado desfasado', 'Declarado desfasado'), ('Pendiente declaración', 'Pendiente declaración')], 'Febrero PLAME', compute='_compute_febrero_plame')
    marzo_plame = fields.Selection([('Declarado a tiempo', 'Declarado a tiempo'), ('Declarado desfasado', 'Declarado desfasado'), ('Pendiente declaración', 'Pendiente declaración')], 'Marzo PLAME', compute='_compute_marzo_plame')
    abril_plame = fields.Selection([('Declarado a tiempo', 'Declarado a tiempo'), ('Declarado desfasado', 'Declarado desfasado'), ('Pendiente declaración', 'Pendiente declaración')], 'Abril PLAME', compute='_compute_abril_plame')
    mayo_plame = fields.Selection([('Declarado a tiempo', 'Declarado a tiempo'), ('Declarado desfasado', 'Declarado desfasado'), ('Pendiente declaración', 'Pendiente declaración')], 'Mayo PLAME', compute='_compute_mayo_plame')
    junio_plame = fields.Selection([('Declarado a tiempo', 'Declarado a tiempo'), ('Declarado desfasado', 'Declarado desfasado'), ('Pendiente declaración', 'Pendiente declaración')], 'Junio PLAME', compute='_compute_junio_plame')
    julio_plame = fields.Selection([('Declarado a tiempo', 'Declarado a tiempo'), ('Declarado desfasado', 'Declarado desfasado'), ('Pendiente declaración', 'Pendiente declaración')], 'Julio PLAME', compute='_compute_julio_plame')
    agosto_plame = fields.Selection([('Declarado a tiempo', 'Declarado a tiempo'), ('Declarado desfasado', 'Declarado desfasado'), ('Pendiente declaración', 'Pendiente declaración')], 'Agosto PLAME', compute='_compute_agosto_plame')
    setiembre_plame = fields.Selection([('Declarado a tiempo', 'Declarado a tiempo'), ('Declarado desfasado', 'Declarado desfasado'), ('Pendiente declaración', 'Pendiente declaración')], 'Setiembre PLAME', compute='_compute_setiembre_plame')
    octubre_plame = fields.Selection([('Declarado a tiempo', 'Declarado a tiempo'), ('Declarado desfasado', 'Declarado desfasado'), ('Pendiente declaración', 'Pendiente declaración')], 'Octubre PLAME', compute='_compute_octubre_plame')
    noviembre_plame = fields.Selection([('Declarado a tiempo', 'Declarado a tiempo'), ('Declarado desfasado', 'Declarado desfasado'), ('Pendiente declaración', 'Pendiente declaración')], 'Noviembre PLAME', compute='_compute_noviembre_plame')
    diciembre_plame = fields.Selection([('Declarado a tiempo', 'Declarado a tiempo'), ('Declarado desfasado', 'Declarado desfasado'), ('Pendiente declaración', 'Pendiente declaración')], 'Diciembre PLAME', compute='_compute_diciembre_plame')
    _rec_name = 'parent_id'
    ...
    def getPdt(self, mes):
        ...
        declaracion = self.pool.get('res.partner.declaraciones').browse(self.env.cr, self.env.uid,  idDeclaracion, context=None)

        id = self.pool.get('res.partner.declaraciones_detalle').search(self.env.cr, self.env.uid,
                                                               [('anio', '=', '2017'),
                                                                ('mes', '=', mes),
                                                                ('parent_detalle_id', '=', declaracion.declaraciones_id.id)])
        detalle = self.pool.get('res.partner.declaraciones_detalle').browse(self.env.cr, self.env.uid, id, context=None)

        if detalle:
            for cro in detalle:
                if cro.fecha_pdt:
                    if cro.fecha_pdt < cro.fecha_limite:
                        return 'Declarado a tiempo'
                    else:
                        return 'Declarado desfasado'
                else:
        ...
        if detalle:
            for cro in detalle:
                if cro.fecha_declarada:
                    if cro.fecha_declarada <= cro.fecha_limite:
                        return 'Declarado a tiempo'
                    else:
                        return 'Declarado desfasado'
                else:
...
class res_partner_reporte_pagos(models.Model):
    ...
    def getImp(self, mes, imp):
        ...
        impuesto = self.pool.get('res.partner.impuestos_pendientes').browse(self.env.cr, self.env.uid,  idImp, context=None)

        id = self.pool.get('res.partner.impuestos_pendientes_detalle').search(self.env.cr, self.env.uid,
                                                               [('anio', '=', '2017'),
                                                                ('mes', '=', mes),
                                                                ('impuesto', '=', imp),
                                                                ('parent_detalle_id', '=', impuesto.impuesto_pendiente_detalle_id.id)])
        detalle = self.pool.get('res.partner.impuestos_pendientes_detalle').browse(self.env.cr, self.env.uid, id, context=None)

        if detalle:
            for cro in detalle:
                if cro.deuda > 0:
                    valor = str(cro.deuda) + "00"
                    return 'Deuda: ' + format(float(valor),'.2f')
                else:
                    return 'No Deuda'
        else:
            return 'No Deuda'
...
class res_partner_reporte_documentos(models.Model):
    ...
    enero_quincena = fields.Selection([('Pendiente', 'Pendiente'), ('Recibido', 'Recibido'), ('No tiene documentos', 'No tiene documentos')], 'Enero Doc Quincena', compute='_compute_enero_quincena')
    febrero_quincena = fields.Selection([('Pendiente', 'Pendiente'), ('Recibido', 'Recibido'), ('No tiene documentos', 'No tiene documentos')], 'Febrero Doc Quincena', compute='_compute_febrero_quincena')
    marzo_quincena = fields.Selection([('Pendiente', 'Pendiente'), ('Recibido', 'Recibido'), ('No tiene documentos', 'No tiene documentos')], 'Marzo Doc Quincena', compute='_compute_marzo_quincena')
    abril_quincena = fields.Selection([('Pendiente', 'Pendiente'), ('Recibido', 'Recibido'), ('No tiene documentos', 'No tiene documentos')], 'Abril Doc Quincena', compute='_compute_abril_quincena')
    mayo_quincena = fields.Selection([('Pendiente', 'Pendiente'), ('Recibido', 'Recibido'), ('No tiene documentos', 'No tiene documentos')], 'Mayo Doc Quincena', compute='_compute_mayo_quincena')
    junio_quincena = fields.Selection([('Pendiente', 'Pendiente'), ('Recibido', 'Recibido'), ('No tiene documentos', 'No tiene documentos')], 'Junio Doc Quincena', compute='_compute_junio_quincena')
    julio_quincena = fields.Selection([('Pendiente', 'Pendiente'), ('Recibido', 'Recibido'), ('No tiene documentos', 'No tiene documentos')], 'Julio Doc Quincena', compute='_compute_julio_quincena')
    agosto_quincena = fields.Selection([('Pendiente', 'Pendiente'), ('Recibido', 'Recibido'), ('No tiene documentos', 'No tiene documentos')], 'Agosto Doc Quincena', compute='_compute_agosto_quincena')
    setiembre_quincena = fields.Selection([('Pendiente', 'Pendiente'), ('Recibido', 'Recibido'), ('No tiene documentos', 'No tiene documentos')], 'Setiembre Doc Quincena', compute='_compute_setiembre_quincena')
    octubre_quincena = fields.Selection([('Pendiente', 'Pendiente'), ('Recibido', 'Recibido'), ('No tiene documentos', 'No tiene documentos')], 'Octubre Doc Quincena', compute='_compute_octubre_quincena')
    noviembre_quincena = fields.Selection([('Pendiente', 'Pendiente'), ('Recibido', 'Recibido'), ('No tiene documentos', 'No tiene documentos')], 'Noviembre Doc Quincena', compute='_compute_noviembre_quincena')
    diciembre_quincena = fields.Selection([('Pendiente', 'Pendiente'), ('Recibido', 'Recibido'), ('No tiene documentos', 'No tiene documentos')], 'Diciembre Doc Quincena', compute='_compute_diciembre_quincena')
    enero_fin = fields.Selection([('Pendiente', 'Pendiente'), ('Recibido', 'Recibido'), ('No tiene documentos', 'No tiene documentos')], 'Enero Doc Fin Mes', compute='_compute_enero_fin')
    febrero_fin = fields.Selection([('Pendiente', 'Pendiente'), ('Recibido', 'Recibido'), ('No tiene documentos', 'No tiene documentos')], 'Febrero Doc Fin Mes', compute='_compute_febrero_fin')
    marzo_fin = fields.Selection([('Pendiente', 'Pendiente'), ('Recibido', 'Recibido'), ('No tiene documentos', 'No tiene documentos')], 'Marzo Doc Fin Mes', compute='_compute_marzo_fin')
    abril_fin = fields.Selection([('Pendiente', 'Pendiente'), ('Recibido', 'Recibido'), ('No tiene documentos', 'No tiene documentos')], 'Abril Doc Fin Mes', compute='_compute_abril_fin')
    mayo_fin = fields.Selection([('Pendiente', 'Pendiente'), ('Recibido', 'Recibido'), ('No tiene documentos', 'No tiene documentos')], 'Mayo Doc Fin Mes', compute='_compute_mayo_fin')
    junio_fin = fields.Selection([('Pendiente', 'Pendiente'), ('Recibido', 'Recibido'), ('No tiene documentos', 'No tiene documentos')], 'Junio Doc Fin Mes', compute='_compute_junio_fin')
    julio_fin = fields.Selection([('Pendiente', 'Pendiente'), ('Recibido', 'Recibido'), ('No tiene documentos', 'No tiene documentos')], 'Julio Doc Fin Mes', compute='_compute_julio_fin')
    agosto_fin = fields.Selection([('Pendiente', 'Pendiente'), ('Recibido', 'Recibido'), ('No tiene documentos', 'No tiene documentos')], 'Agosto Doc Fin Mes', compute='_compute_agosto_fin')
    setiembre_fin = fields.Selection([('Pendiente', 'Pendiente'), ('Recibido', 'Recibido'), ('No tiene documentos', 'No tiene documentos')], 'Setiembre Doc Fin Mes', compute='_compute_setiembre_fin')
    octubre_fin = fields.Selection([('Pendiente', 'Pendiente'), ('Recibido', 'Recibido'), ('No tiene documentos', 'No tiene documentos')], 'Octubre Doc Fin Mes', compute='_compute_octubre_fin')
    noviembre_fin = fields.Selection([('Pendiente', 'Pendiente'), ('Recibido', 'Recibido'), ('No tiene documentos', 'No tiene documentos')], 'Noviembre Doc Fin Mes', compute='_compute_noviembre_fin')
    diciembre_fin = fields.Selection([('Pendiente', 'Pendiente'), ('Recibido', 'Recibido'), ('No tiene documentos', 'No tiene documentos')], 'Diciembre Doc Fin Mes', compute='_compute_diciembre_fin')
