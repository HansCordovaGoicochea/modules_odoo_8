...
def firma(valorFirma, certificado):
    ...
    dsCanonicalizationMethod.setAttribute('Algorithm',
...
def id():
    cbcID = doc.createElement('cbc:ID')
    text = doc.createTextNode('FF11-01')
    cbcID.appendChild(text)
...
def referenceDate(fecha):
    cbcIssueDate = doc.createElement('cbc:ReferenceDate')
    text = doc.createTextNode(fecha)
    cbcIssueDate.appendChild(text)
    xml_str = cbcIssueDate.toprettyxml(indent="  ")
    return xml_str
...
def voidedDocumentsLine(id, type, serial, number, reason):
    sacVoidedDocumentsLine = doc.createElement('sac:VoidedDocumentsLine')
    cbcLineID = doc.createElement('cbc:LineID')
    text = doc.createTextNode(id)
    ...
    sacDocumentSerialID = doc.createElement('sac:DocumentSerialID')
    text = doc.createTextNode(serial)
    sacDocumentSerialID.appendChild(text)
    sacVoidedDocumentsLine.appendChild(sacDocumentSerialID)
    sacDocumentNumberID = doc.createElement('sac:DocumentNumberID')
    text = doc.createTextNode(number)
    sacDocumentNumberID.appendChild(text)
    sacVoidedDocumentsLine.appendChild(sacDocumentNumberID)
    sacVoidReasonDescription = doc.createElement('sac:VoidReasonDescription')
    text = doc.createTextNode(reason)
    sacVoidReasonDescription.appendChild(text)
    sacVoidedDocumentsLine.appendChild(sacVoidReasonDescription)
    xml_str = sacVoidedDocumentsLine.toprettyxml(indent="  ")
    return xml_str
...
n = customizationID()
o = id()
e = issueDate('2016-11-24')
t = referenceDate('2016-11-10')
g = invoiceTypeCode()
h = documentCurrencyCode()
i = declaracionFirma('id', 'identificacion', 'name', 'firma')
j = DatosProveedor('id', 'idAdicional', 'mame', 'idDireccion', 'addres', 'nombreCiudad',
                   'nombreDepartamento', 'name', 'nombreRegistro'
                   , 'codigoID')
k = DatosCliente('id', 'idAdicional', 'nombre')
l = sumatoriaIGV('12')
q = importeTotal('0.00')
m = Producto('idP', '1', '0.00', '0.00', '001', 'identificacion', '10.00')
r = discrepancyResponse('FF11-02', '07', 'Comentario de la nota de debito')
s = billingReference('FF11-02', '01')
z = voidedDocumentsLine('1', '01', 'FF01', '901', 'ANULACIONDE LA FACTURA 1')
