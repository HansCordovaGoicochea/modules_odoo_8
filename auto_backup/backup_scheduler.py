...
class db_backup(osv.Model):
    ...
    _columns = {
        ...
        'daystokeepsftp': fields.integer('Remove SFTP after x days',
                                         help="Choose after how many days the backup should be deleted from the FTP server. For example:\nIf you fill in 5 the backups will be removed after 5 days from the FTP server."),
        'sendmailsftpfail': fields.boolean('Auto. E-mail on backup fail',
                                           help="If you check this option you can choose to automaticly get e-mailed when the backup to the external server failed."),
        'emailtonotify': fields.char('E-mail to notify',
                                     help="Fill in the e-mail where you want to be notified that the backup failed on the FTP."),
        'sendmailbackup': fields.boolean('Auto. E-mail Backup',
                                           help="Si tu marcas esta opcion el backup se enviara automaticamente al correo que coloques en la caja de texto."),
        'emailtobackup': fields.char('Enviar Backup por Em-mail',
                                     help="Color el correo a donde necesita enviar el backup"),
    ...
    # @api.multi
    # def _cron_horario_backups(self):
    #     self.schedule_backup()

    def schedule_backup(self, cr, user, ids=None, context=None):
        print ('dddddddddddddddddddddddfdfdfdfdf')
        ...
        for rec in confs:
            ...
            if rec.name in db_list:
                ...
                try:
                    ...
                    logger.debug("Couldn't backup database %s. Bad database administrator password for server running at http://%s:%s" % (rec.name, rec.host, rec.port))
                    continue
                    ...
                    if rec.sendmailsftpfail:
                        try:
                            ...
                            message = "Dear,\n\nThe backup for the server " + rec.host + " (IP: " + rec.sftpip + ") failed.Please check the following details:\n\nIP address SFTP server: " + rec.sftpip + "\nUsername: " + rec.sftpusername + "\nPassword: " + rec.sftppassword + "\n\nError details: " + tools.ustr(
                                e) + "\n\nWith kind regards"
                            msg = ir_mail_server.build_email("auto_backup@" + rec.name + ".com", [rec.emailtonotify],
                                                             "Backup from " + rec.host + "(" + rec.sftpip + ") failed",
                                                             message)
    ...
    def enviar_email_por_correo(self, cr, user, ids=None, context=None):
        print ('enviar_email_por_correodsdddddddddddd')
        conf_ids = self.search(cr, user, [])
        confs = self.browse(cr, user, conf_ids)
        for rec in confs:
            if rec.sendmailbackup:
                try:
                    ir_mail_server = self.pool.get('ir.mail_server')
                    smtp_email_from_ids = ir_mail_server.search(cr, user,[('active','=', True)])
                    print (smtp_email_from_ids)
                    smtp_email_from = ir_mail_server.browse(cr, user,smtp_email_from_ids)[0]
                    print (smtp_email_from)
                    message = "Estimado,\n\nEl backup de la base de datos " + rec.name + "\n\ncon cordiales saludos"
                    dir = rec.bkp_dir
                    # Loop over all files in the directory.
                    for f in os.listdir(dir):
                        fullpath = os.path.join(dir, f)
                        ...
                        if createtime.day == now.day:
                            # Only seleccionar files (which are .dump), no directories.
                            if os.path.isfile(fullpath) and ".dump" in f:
                                print("Enviar este archivo: " + fullpath)
                                archivo = fullpath.replace('.dump','')+'.zip'
                                # archivo = fullpath
                                import zipfile
                                jungle_zip = zipfile.ZipFile(archivo, 'w')
                                jungle_zip.write(fullpath, arcname=f)
                                jungle_zip.close()
                                g = open(r'' + archivo, 'rb')
                                data_file = g.read()
                                b64 = base64.b64encode(str(data_file))

                                msg = ir_mail_server.build_email(email_from=smtp_email_from.smtp_user, email_to=[rec.emailtobackup], subject="Backup from " + rec.host, body=message, attachments=[(str(f), b64)])
                                ir_mail_server.send_email(cr, user, msg)
                                g.close()
                except Exception, e:
                    print ('No se envio el Error es!', str(tools.ustr(e)))
