...
class fleet_documento_vehiculo(models.Model):
    def fields_view_get(self, cr, uid, view_id=None, view_type='form',
        ...
        for r in re:
            ...
            new_date = dateutil.parser.parse(r['fecha_vencimiento']).date() - datetime.timedelta(days=r['nro_dias_antes_alerta'])
            if cur_date >= new_date:
                # print ('consulta update True')
                cr.execute("""UPDATE fleet_documento_vehiculo SET check_date = %s where fecha_vencimiento=%s""", (True, r['fecha_vencimiento']))
            else:
                cr.execute("""UPDATE fleet_documento_vehiculo SET check_date = %s where fecha_vencimiento=%s""", (False, r['fecha_vencimiento']))
    ...
    documentos = fields.Many2many('ir.attachment', 'class_vehiculo_ir_attachments_rel', 'class_id', 'attachment_id','Documento')
...
class modulo_valorizaciones_revision_vehiculo(models.Model):
    ...
    _inherits = {'modulo_valorizaciones.revision_vehiculo_detalle': 'revision_tecnica_detalle_id', 'modulo_valorizaciones.proximo_mantenimiento': 'proximo_mantenimiento_id'}
...
class modulo_valorizaciones_revision_vehiculo_detalle(models.Model):
    ...
    revision_tecnica_detalle_ids = fields.One2many('modulo_valorizaciones.revision_vehiculo_detalle', 'parent_detalle_id', 'Included Services')
...
class modulo_valorizaciones_proximo_mantenimiento(models.Model):
    ...
    parent_proximo_id = fields.Many2one('modulo_valorizaciones.proximo_mantenimiento', 'Parent',
                                help='Parent cost to this current cost')
    proximo_mantenimiento_ids = fields.One2many('modulo_valorizaciones.proximo_mantenimiento', 'parent_proximo_id', 'Included Services')
...
class modulo_valorizaciones_revision_tecnica(models.Model):
    ...
    estado = fields.Selection([('Pendiente', 'Pendiente'), ('Programado', 'Programado'), ('Realizado', 'Realizado'), ('Pagado', 'Pagado')], 'Estado', default='Programado')
    observaciones = fields.Text('Observaciones')
    ...
    tipo_vehiculo_revision_tecnica = fields.Many2one('modulo_valorizaciones.tipo_unidad', 'Tipo Unidad', default=_name_get_tipo_unida)
...
class modulo_valorizaciones_tipo_valorizacion(models.Model):
    ...
    adicional_id = fields.Many2many('modulo_valorizaciones.adicionales', 'modulo_valorizaciones_adicionales_rel', 'adicional_tipo_valorizacion_id', 'adicional_id', 'Incluye', copy=False)
...
class fleet_vehicle_log_contract(models.Model):
    ...
    tipo_valorizacion_id = fields.Many2one('modulo_valorizaciones.tipo_valorizacion', 'Tipo Valorizacion', required=False)
...
class modulo_valorizaciones_vehiculo_contrato_detalle(models.Model):
    ...
    parent_id = fields.Many2one('fleet.vehicle.log.contract', 'Parent', help='Parent cost to this current cost', ondelete='cascade')
    # contrato_ids = fields.One2many('modulo_valorizaciones.vehiculo_contrato_detalle', 'parent_id', 'Included Services')
...
class modulo_valorizacion_tareo_mensual(models.Model):
    ...
    def _compute_iframe(self):
        if self.id:
            self.iframe = '<iframe marginheight="0" id="frameValorizaciones" marginwidth="0" frameborder = "0" src="http://192.168.2.44:81/valorizaciones/home/index/c/'+str(self.id)+'" width="100%" height="1000"/>'
        else:
            self.iframe = '<iframe marginheight="0" id="frameValorizaciones" marginwidth="0" frameborder = "0" src="http://192.168.2.44:81/valorizaciones/home/index/c/0" width="100%" height="1000"/>'
    ...
    iframe_create = fields.Html('Embedded Webpage', default='<iframe marginheight="0" id="frameValorizaciones" marginwidth="0" frameborder = "0" src="http://192.168.2.44:81/valorizaciones/home/index/c/0" width="100%" height="1000"/>', sanitize=False, strip_style=False, readonly=True)
    ...
    def unlink(self):
        for dd in self:
            ...
            self._cr.execute("""SELECT * FROM "fleet_vehicle_log_contract" JOIN "modulo_valorizaciones_vehiculo_contrato_detalle" ON "fleet_vehicle_log_contract"."id" = "modulo_valorizaciones_vehiculo_contrato_detalle"."parent_id" WHERE "fleet_vehicle_log_contract"."id" = %s""", [dd.contrato.id])
            r = self._cr.dictfetchall()
            # print (r)
            for resultado in r:
                # self._cr.execute("""DELETE FROM modulo_valorizaciones_tareo WHERE fecha >= %s and fecha <= %s and idruta= %s and idunidad = %s and tipo=%s and cambio=%s""",(dd.fechaInicio,dd.fechaFin,resultado['ruta_id'],resultado['unidad_id'], 'v',True))
                self._cr.execute("""DELETE FROM modulo_valorizaciones_tareo WHERE fecha >= %s and fecha <= %s and idruta= %s and idunidad = %s""",(dd.fechaInicio,dd.fechaFin,resultado['ruta_id'],resultado['unidad_id']))
...
class modulo_valorizaciones_sam(models.Model):
    ...
    def facturar_tasks(self, context=None):
        ...
        for fac in self:
            ...
            account_detalle_id = self.pool.get('account.account').search(self.env.cr, self.env.uid, [('code', '=', '7041')])
...
class modulo_valorizaciones_sam_detalle(models.Model):
    ...
    def check_change_fecha(self):
        ...
        if self.fecha:
            ...
            elif str(self.fecha) > str(fecha_fin):
    ...
    observaciones = fields.Text('Observaciones')
    parent_id = fields.Many2one('modulo_valorizaciones.sam_detalle', 'Parent')
...
class fleet_vehicle_log_fuel(models.Model):
    ...
    estado = fields.Selection([('Creado', 'Creado'), ('Cancelado', 'Cancelado'), ('Facturado', 'Facturado')], 'Estado', default='Creado')
        ...
        if 'proveedor' in vals and 'monto_dinero' in vals and 'monto_galones' in vals and 'amount' in vals:
            if vals['monto_dinero'] > 0 and vals['monto_galones'] > 0:
                self._cr.execute(""" UPDATE res_partner SET monto=%s, combustible=%s WHERE id=%s""",
                                 (vals['monto_dinero']-vals['amount'], vals['monto_galones']-vals['liter'], vals['proveedor']))
            else:
                self._cr.execute(""" UPDATE res_partner SET monto=%s, combustible=%s WHERE id=%s""",
                                 (0, 0, vals['proveedor']))
        ...
        if liter:
            ...
            if monto_galones > 0:
                if liter > monto_galones:
                    ...
                    return {'value': v, 'warning':{'title':'Error','message':'No tiene disponible la cantidad solicitada'}}
                else:
...
class modulo_valorizaciones_linea_credito(models.Model):
    ...
    def check_change_litros(self):
        ...
        if self.proveedor.id:
            self._cr.execute(""" UPDATE res_partner SET combustible=%s WHERE id=%s""", (self.monto_galones, self.proveedor.id))
...
class modulo_valorizaciones_pantalla_especifica(models.Model):
    ...
    def _get_unidad2(self):
        pantalla_especifica = self.env['modulo_valorizaciones.pantalla_general'].search([('id', '=', ids_tramo_general)])
    ...
    parent_pant_espec_id = fields.Many2one('modulo_valorizaciones.pantalla_general', 'Tramo General', ondelete="cascade")
...
class modulo_valorizaciones_pantalla_general(models.Model):
    ...
    horario_general_ids = fields.One2many('modulo_valorizaciones.horario_general', 'parent_horario_id', 'Included Services', copy=True)
    kilometraje_general_ids = fields.One2many('modulo_valorizaciones.kilometraje_general', 'parent_kilometraje_id', 'Included Services', copy=True)
...
class modulo_valorizaciones_horario_general(models.Model):
        ...
        if vals['fecha_cambio']:
            ...
            if vals['secuencia']:
                ...
                if vals['horario'] and unidad and ruta_id:
                    self._cr.execute("""SELECT dias_descanso, dias_trabajo FROM modulo_valorizaciones_horario WHERE id = %s""", [vals['horario']])
    ...
    def unlink(self):
        ...
        horario_general = self.env['modulo_valorizaciones.horario_general'].search([('parent_horario_id','=',ids_tramo_general)], limit=1, order=sortBy)
        ...
        if horario_general:
            ...
            if horario_general.fecha_cambio:
                ...
                # if vals.has_key('fecha_cambio'):
                # d1 = date(int(self.fecha_cambio[0:4]), int(self.fecha_cambio[5:7]), int(self.fecha_cambio[-2:]))
                d1 = date(int(horario_general.fecha_cambio[0:4]), int(horario_general.fecha_cambio[5:7]), int(horario_general.fecha_cambio[-2:]))
                d2 = date(int(dateMonthEnd[0:4]), int(dateMonthEnd[5:7]), int(dateMonthEnd[-2:]))
                diff = d2 - d1
...
class modulo_valorizaciones_proveedor(models.Model):
    ...
    def _compute_iframe(self):
        if self.id:
            self.iframe = '<iframe marginheight="0" id="frameValorizaciones" marginwidth="0" frameborder = "0" src="http://192.168.2.44:81/valorizaciones/home/index/p/' + str(
                self.id) + '" width="100%" height="1000"/>'
        else:
            self.iframe = '<iframe marginheight="0" id="frameValorizaciones" marginwidth="0" frameborder = "0" src="http://192.168.2.44:81/valorizaciones/home/index/p/0" width="100%" height="1000"/>'
    ...
    n_factura = fields.Many2one('account.invoice', 'N° Factura')
    nota = fields.Text('Nota')
    estado = fields.Selection([('No creado', 'No creado'), ('Pagado', 'Pagado'), ('Facturado', 'Facturado'), ('No Emitido', 'No Emitido'), ('Cancelado', 'Cancelado')], 'Estado', default='No creado')
    _rec_name = 'proveedor'
    creado = fields.Boolean('creado', default=False, compute='_compute_creado')
    iframe = fields.Html('iframe', compute='_compute_iframe',
                         sanitize=False, strip_style=False)
    iframe_create = fields.Html('iframe Create',
                                default='<iframe marginheight="0" id="frameValorizaciones" marginwidth="0" frameborder = "0" src="http://localhost:81/valorizaciones/home/index/p/0" width="100%" height="1000"/>',
...
class val_fleet_service_type(models.Model):
    ...
    category = fields.Selection([('contract', 'Contract'), ('service', 'Service'), ('both', 'Both'), ('documento', 'Documento')], 'Category',required=True,help='Choose wheter the service refer to contracts, vehicle services or both')
...
class modulo_valorizaciones_cambio_usuario(models.Model):
    ...
    contrato_detalle_id = fields.Many2one('modulo_valorizaciones.vehiculo_contrato_detalle')
    ruta_id = fields.Many2one('modulo_valorizaciones.ruta', 'Ruta')
    unidad_id = fields.Many2one('fleet.vehicle', 'Vehiculo')
    fechaFacturacion = fields.Date('Fecha Facturación')
